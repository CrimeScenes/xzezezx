...
RunService.RenderStepped:Connect(function()
    ...
    -- Check for ThirdPersonLockout condition
    if getgenv().CamLock.Normal.ThirdPersonLockout then
        local cameraMode = game:GetService("Players").LocalPlayer.CameraMode
        local isShiftLocked = game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.LeftShift)

        -- If in third-person and ShiftLock is not active, disable CamLock
        if cameraMode == Enum.CameraMode.Classic and not isShiftLocked then
            IsTargeting = false  -- Disable targeting in third-person view without ShiftLock
            return
        end
    end
